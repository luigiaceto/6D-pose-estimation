...
class ResNetPose(nn.Module):
    ...
    def _init_weights(self):
        ...
        for layer in self.fc_layers_r.modules():
        ...
        # Output Head (quaternion_head) -> Lineare (no ReLU) -> Xavier Init
        # Usiamo gain=0.01 per partire con pesi piccoli e output stabile
        nn.init.xavier_uniform_(self.quaternion_head.weight, gain=0.01)
        
        # Bias: Quaternione identità [1, 0, 0, 0]
        nn.init.constant_(self.quaternion_head.bias, 0.0)
        with torch.no_grad():
            self.quaternion_head.bias[0] = 1.0
