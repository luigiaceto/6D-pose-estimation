...
def draw_2d_bbox(img, box, color, label, thickness=3):
    ...
    cv2.putText(img, label, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, lineType=cv2.LINE_AA)
    return img
...
def draw_3d_bbox_colored(img, R, t, K, obj_id, models_info, color=(255, 255, 0)):
    ...
    for e in edges:
        if e[0] < len(corners_2d) and e[1] < len(corners_2d):
            cv2.line(img, corners_2d[e[0]], corners_2d[e[1]], color, 2, lineType=cv2.LINE_AA)
